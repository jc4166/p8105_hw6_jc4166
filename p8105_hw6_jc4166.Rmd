...
Based on my experience as a clinician, I propose a model in which low birthweight can be predicted by gestational age in weeks, mother's smoking status, history of previous low birth weight babies, and family income.  
...
Model 1 has the lowest prediction error of the three models. Model 3 appears to be a close second, though clinically I don't think it makes sense to have a prediction model that includes measurements that can taken after the baby is born (i.e. baby's length and head circumference).
...
Distribution of two quantities estimated from these data: r^2  and log(β̂ 0∗β̂ 1). 
